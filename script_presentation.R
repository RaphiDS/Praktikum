...
# Berechnung der Stichprobengröße pro Kategorie
# Dependency als Faktor mit der gewünschten Reihenfolge definieren
Drug.Dependency.Total <- Drug.Dependency.Total %>%
  mutate(Dependency = factor(Dependency, levels = c("Keine Abhängigkeit", "Alkohol", "Kokain", "Heroin", "Mehrfachabhängigkeit")))

# Stichprobengröße pro Kategorie berechnen
sample_sizes_ddt <- Drug.Dependency.Total %>%
  group_by(Dependency) %>%
  summarise(Sample_Size_ddt = sum(count), .groups = "drop") %>%
  mutate(Label = paste0("n = ", Sample_Size_ddt))  # Format für die Annotation

# Plot mit angepasster x-Achse und korrekter Zuordnung der Stichprobengröße
SubsAbhängig.Gesundheit <- ggplot(Drug.Dependency.Total, aes(x = Dependency, fill = factor(MI_CAT_U))) +
